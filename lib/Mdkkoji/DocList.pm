...
QUERY
	...
	$update_sth->execute( ($score, $excerpt) = $sub->({
		query => $self->{query},
		path  => $path, 
		ref   => $ref, 
		title => $title, 
...
=head1 SYNOPSIS
  ...
  $list = DocList->new($dbh, $query, @options);
  $list->filter(sub{
      my $args = shift;
      # query => 
      # path => 
...
이 서브루틴이 넘겨 받는 인자는 다음과 같은 해시 참조입니다.

  {
	  query => { # 생성자에 넘긴 $query }, 
	  path => '문서의 경로', 
	  ref => '문서의 경로와 짝 지을 URL', 
	  title => '문서의 제목', 
	  score => undef, # 점수. 초기 값은 undef
	  excerpt => '', # 발췌문
  }

이 서브루틴은 점수와 발췌문을 돌려줍니다. 

모든 문서를 검토한 뒤에는 점수가 0인 문서를 목록에서 제외합니다. 생성자에게 인자로
넘겨주는 검색 조건은 점수를 따로 매기지 않습니다. 따라서 이 메서드를 처음 호출할
때에는 C<$score>의 값이 C<undef>일 것입니다. 점수가 C<undef>면 목록에서 제외되지 않습니다.

지정한 점수와 발췌문은 이 메서드를 다시 호출 할 때에도 그대로 남아 있습니다.
